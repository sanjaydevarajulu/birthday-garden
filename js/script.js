...
function renderPhotos(){
  ...
  updateGalleryCount();
  stage.onclick=event=>{const card=event.target.closest("[data-photo]");if(card){currentPhoto=Number(card.dataset.photo);updateGalleryCount();openPhoto(currentPhoto);}};
  ...
  stage.addEventListener("scroll",updateGalleryFromScroll,{passive:true});
}
function updateGalleryCount(){const count=$("#galleryCount");if(count)count.textContent=`${String(currentPhoto+1).padStart(2,"0")} / ${String(photos.length).padStart(2,"0")}`;}
function updateGalleryFromScroll(){const stage=$("#photoStage");if(!stage||!photos.length)return;const center=stage.getBoundingClientRect().left+stage.clientWidth/2;let closest=0,distance=Infinity;$$('.photo-card',stage).forEach((card,i)=>{const d=Math.abs(card.getBoundingClientRect().left+card.clientWidth/2-center);if(d<distance){distance=d;closest=i;}});currentPhoto=closest;updateGalleryCount();}
function movePhoto(delta){if(!photos.length)return;currentPhoto=(currentPhoto+delta+photos.length)%photos.length;const card=$(`[data-photo="${currentPhoto}"]`);card?.scrollIntoView({behavior:reducedMotion?"auto":"smooth",block:"nearest",inline:"center"});updateGalleryCount();}function openPhoto(index){currentPhoto=index;updateGalleryCount();const photo=photos[index];if(!photo)return;const image=$("#lightboxImage");image.src=photo.image;image.alt=`${photo.title}: ${photo.caption}`;image.onerror=()=>{image.removeAttribute("src");image.alt="Photo unavailable";};$("#lightboxCaption").textContent=[photo.title,photo.caption,photo.finalNote].filter(Boolean).join("\n\n");$("#lightbox").showModal();}
