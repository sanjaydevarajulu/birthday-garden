...
function renderPhotos(){
  const stage=$("#photoStage");if(!photos.length){stage.innerHTML='<div class="player-empty">Add your favourite photographs to the <code>photos</code> list in <code>js/script.js</code>.</div>';$("#galleryCount").textContent="Your memories will appear here";return;}
  const visible=photos.map((_,i)=>(i-currentPhoto+photos.length)%photos.length).sort((a,b)=>a-b);
  stage.innerHTML=photos.map((photo,i)=>`<figure class="photo-card" data-photo="${i}" role="button" tabindex="0" aria-label="View ${safeText(photo.title)}: ${safeText(photo.caption)}"><img loading="lazy" src="${safeText(photo.image)}" alt="${safeText(photo.title)} — ${safeText(photo.caption)}" onerror="this.outerHTML='<div class=&quot;photo-placeholder&quot;>your lovely<br>moment ✦</div>'"><figcaption><strong>${safeText(photo.title)}</strong><span>${safeText(photo.caption)}</span></figcaption></figure>`).join("");
  // Place the active photograph in front, with its neighbours fanned around it.
  $$(".photo-card",stage).forEach((card,i)=>{const delta=((i-currentPhoto+photos.length)%photos.length);const normalized=delta>photos.length/2?delta-photos.length:delta;card.style.zIndex=String(10-Math.abs(normalized));card.style.transform=`translateX(${normalized*68-50}%) translateY(${Math.abs(normalized)*4}px) rotate(${normalized*7}deg)`;card.style.opacity=Math.abs(normalized)>2?"0":"1";});
  $("#galleryCount").textContent=`${String(currentPhoto+1).padStart(2,"0")} / ${String(photos.length).padStart(2,"0")}`;
  stage.onclick=event=>{const card=event.target.closest("[data-photo]");if(card){currentPhoto=Number(card.dataset.photo);renderPhotos();openPhoto(currentPhoto);}};
  stage.onkeydown=event=>{if((event.key==="Enter"||event.key===" ")&&event.target.matches("[data-photo]")){event.preventDefault();openPhoto(Number(event.target.dataset.photo));}};
}
function movePhoto(delta){if(!photos.length)return;currentPhoto=(currentPhoto+delta+photos.length)%photos.length;renderPhotos();}
function openPhoto(index){currentPhoto=index;const photo=photos[index];if(!photo)return;const image=$("#lightboxImage");image.src=photo.image;image.alt=`${photo.title}: ${photo.caption}`;image.onerror=()=>{image.removeAttribute("src");image.alt="Photo unavailable";};$("#lightboxCaption").textContent=[photo.title,photo.caption,photo.finalNote].filter(Boolean).join("\n\n");$("#lightbox").showModal();}
function renderLetters(){const list=$("#letterList");list.innerHTML=letters.map((letter,i)=>`<button class="envelope" type="button" data-letter="${i}" aria-label="Open letter: ${safeText(letter.title)}"><span class="envelope-art"><span class="envelope-flap"></span><span class="envelope-seal">♡</span></span><span class="envelope-label">${safeText(letter.title)}</span><small>${safeText(letter.label)}</small></button>`).join("");list.addEventListener("click",event=>{const button=event.target.closest("[data-letter]");if(!button)return;const letter=letters[Number(button.dataset.letter)];openContent(`<div class="section-kicker"><span>♡</span><i></i> A letter, just for you</div><h2>${safeText(letter.title)}</h2><p>${safeText(letter.text)}</p><p class="message-signoff">Always, <span>♡</span></p>`);});}
...
function setupPage(){
  ...
  $("#galleryPrev").addEventListener("click",()=>movePhoto(-1));$("#galleryNext").addEventListener("click",()=>movePhoto(1));
